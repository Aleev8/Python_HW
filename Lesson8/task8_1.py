...
receips = {'Эспрессо': [1, 0, 0],
           'Капучино': [1, 3, 0],
           'Маккиато': [2, 1, 0],
           'Кофе по-венски': [1, 0, 2],
           'Латте Маккиато': [2, 0, 1],
           'Кон Панна': [1, 0, 1]
}

# Пополняем склад продуктов
print('Введите количество ингридиентов, имеющееся на складе в порциях:')
print('через пробел - кофейные зерна, молоко, взбитые сливки')
ingredients = list(input().strip().split(' '))
#print(ingredients)
print()

# Выводим меню и вводим список предпочтений клиента
print('Наш бариста умеет готовить следующие виды кофе: ')
print(*receips.keys(), sep='\n')
print()
print('Введите (через пробел) названиz кофе в порядке убывания вашего предпочтения: ')
prefer = list(input().split(' '))
print(prefer)

again = 'Д'
while again == 'Д':
    result = chooce_coffee(prefer)
    print('Из вашего выбора и имеющихся ингридиентов получился:', result)
